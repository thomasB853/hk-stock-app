...
# ================== 页面UI ==================
st.title("📈 港股分析預測系統｜最終版")
st.markdown("### 支持：騰訊、美團、匯豐等主流港股 + 恆生指數（^HSI）")
...
def last_year_performance_analysis(stock_name):
    """本地业绩模板，彻底解决API获取失败问题"""
    st.subheader("📊 去年財務業績（2024年度，本地模板）")
...
def get_hk_stock_data(symbol):
    ...
    try:
        ...
        st.success(f"✅ 數據獲取成功！共 {len(df)} 條交易記錄")
        return df
    except Exception as e:
        st.error(f"❌ 數據獲取異常：{str(e)[:50]}")
        return None
...
# ================== 主执行逻辑 ==================
if st.button("🚀 開始分析（最終版）", type="primary", use_container_width=True):
    if user_code != "^HSI" and (not user_code.isdigit() or len(user_code) != 4):
        ...
        if user_code == "^HSI":
            ...
            st.info("""
            恆生指數（^HSI）作為香港市場核心指數，其走勢受全球宏觀經濟、資金流向、政策變化等多重因素影響：
            1. 短期走勢：基於技術指標（MA線、RSI）和歷史數據，模型給出了未來走勢預測；
            2. 中長期趨勢：需結合全球經濟復蘇進度、中美關係、香港本地政策等基本面因素；
            3. 風險提示：指數波動劇烈，預測僅為技術面參考，不構成投資建議。
            """)
        ...
        st.warning("2. 數據來源為Yahoo Finance，業績數據為本地模板，請以港交所官方發布的數據為準；")
...
# ================== 底部信息 ==================
st.divider()
st.caption("✅ 港股分析預測系統｜最終版")
st.caption("核心功能：全周期均線MA5/20/30/50/100 + 價格/指數預測（置信區間25%） + 去年業績分析 + 恆生指數走勢預測")
st.caption("兼容環境：Python 3.10+/3.12+（Windows/Mac/Linux/Streamlit Cloud）｜中文正常顯示｜無API依賴")
